...
适用于起点、番茄、微信读书、部分笔趣阁、晋江、飞卢、部分轻小说站等
...
本脚本为独立开发的用户脚本项目，与其适配或运行的网站不存在任何隶属、合作、授权或官方关联关系。

本脚本主要用于调整网页在用户本地浏览器中的显示效果、界面布局或交互体验，以提升使用便利性；除明确说明外，不主动修改网站服务器提供的原始内容数据，也不提供、托管或分发相关网站内容。

相关网站展示的内容及其版权归对应的网站运营方、内容提供方或权利人所有。用户在使用本脚本时，仍应遵守相关网站的服务条款、版权规定及适用法律法规。涉及内容复制、转载、下载、传播等行为的，用户应自行确认其合法性并承担相应责任。

用户因使用本脚本违反相关网站规则、服务条款或适用法律法规所产生的责任，由用户自行承担。

`Excel模式`下的部分界面伪装文本为程序随机生成，仅用于视觉效果展示；若生成内容在偶然情况下具有现实含义或与真实信息形成巧合，纯属随机结果，并不代表作者的任何立场、意图或事实陈述。
